...
**If spec.md is missing:** Cannot proceed with evolution. Use `sdd:spec` first.

**If plan.md or tasks.md exist:**
- These were generated by spec-kit and must be kept in sync
- After modifying the spec, regenerate dependent artifacts:
  ```bash
  specify plan specs/[feature-name]/spec.md   # Regenerate plan
  specify tasks specs/[feature-name]/spec.md  # Regenerate tasks
  specify analyze specs/[feature-name]/       # Verify consistency
  ```

**CRITICAL: Never manually edit plan.md or tasks.md. Always regenerate via spec-kit after spec changes.**
...
if [ -f "$SPEC_DIR/plan.md" ]; then
  echo "Regenerating plan.md..."
  specify plan "$SPEC_DIR/spec.md"
fi

if [ -f "$SPEC_DIR/tasks.md" ]; then
  echo "Regenerating tasks.md..."
  specify tasks "$SPEC_DIR/spec.md"
fi

# Verify consistency across all artifacts
specify analyze "$SPEC_DIR/"
```

**VERIFICATION CHECKPOINT:**

```bash
# Verify artifacts are in sync after evolution
if [ -f "$SPEC_DIR/plan.md" ] || [ -f "$SPEC_DIR/tasks.md" ]; then
  specify analyze "$SPEC_DIR/" 2>&1
  if [ $? -ne 0 ]; then
    echo "❌ Artifacts out of sync after evolution"
    echo "Run specify plan and specify tasks to regenerate"
  else
    echo "✓ All artifacts verified in sync"
  fi
fi
```
